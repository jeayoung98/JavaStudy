...
public class EncodingMain2 {
    ...
    private static void test(String text, Charset endcodingCharset, Charset decodingCharset) {
        byte[] encoded = text.getBytes(endcodingCharset);
        String decoded = new String(encoded, decodingCharset);
        System.out.printf("%s -> [%s] 인코딩 -> %s %sbyte -> [%s] 디코딩 -> %s\n",
                text, endcodingCharset, Arrays.toString(encoded), encoded.length, decodingCharset, decoded);
    }
}
